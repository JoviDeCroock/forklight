...
**Session.** `src/middleware/session.ts` mints an anonymous session cookie on first page load. WebMCP tool calls dispatch through the same-origin HTTP projection with credentials, so the agent operates on the very incident the human is looking at — same tab, same row in D1, no account, no login. A capability dispatch arriving without a session is refused: tools do not mint sessions, pages do. Sessions expire by idleness, not by logout: 45 minutes without an interaction and the next visit reseeds the pristine incident, so reloads and long agent runs resume seamlessly while a fresh launch never opens mid-story.
