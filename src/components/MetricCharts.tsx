...
const TILE_LABELS: Record<string, string> = {
  web_p95_ms: "p95 latency",
  cache_hit_ratio: "Cache hit ratio",
  requests_per_min: "Requests",
  db_cpu: "Primary DB CPU",
};
...
function isDegraded(chart: ChartData): boolean {
  ...
  if (chart.id === "web_error_rate") return chart.now >= 1;
  return chart.id === "cache_hit_ratio" || chart.id === "requests_per_min"
...
function HeroChart({
  ...
  const sloVisible = chart.id === "web_error_rate" && 1 >= s.v0 && 1 <= s.v1;
