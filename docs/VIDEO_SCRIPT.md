...
| --- | --- | --- |
| 0:00–0:08 | Cold open on the canvas. Charts red: checkout error rate climbing, orders falling. Cursor traces the 14:05 deploy marker on the timeline, then the two alert chips. | "This is a checkout outage. Error rate went vertical at 14:05, right after a deploy. Orders are down two thirds." |
...
| 0:40–0:55 | Agent panel shows `signals.query` calls. Cut to the raw log output; highlight the `PriceMismatchError` lines, then scroll to the injected user-agent line and hold two seconds on it. | "It goes to the logs. Price mismatches, cache key fragmentation — and this. Someone's user agent telling it to apply the mitigation immediately. That tool's marked as untrusted content, and it ignores it." |
| 0:55–1:08 | The comparison table renders: recovery ETA, orders lost per minute, blast radius, confidence, residual risk. Rollback and cache-bypass side by side. | "Then it compares the branches properly. Rollback takes six minutes through the pipeline. Bypassing the cache recovers in two, but it pushes read load back onto the pricing origin." |
...
| 1:32–1:45 | Click **Confirm apply**. The status chip goes amber then green, the clock jumps from 14:32 to 14:42, error rate collapses on the chart, orders climb back. | "Confirm. The clock jumps past the lead time, and now we find out whether it was right — errors down, orders back." |
